...
void Renderer::LoadPipeline()
{
	...
	ThrowIfFailed(SUCCEEDED(D3D12CreateDevice(hardware_adapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&device))));
	...
	ThrowIfFailed(dxgi_factory->CreateSwapChainForHwnd(command_queue.Get(), Win32Window::GetHwnd(), 
		&swap_chain_descriptor, nullptr, nullptr, &temp_swap_chain));
	...
	for (UINT i = 0; i < frame_number; i++) {
		...
		render_targets[i]->SetName((L"Render Target " + std::to_wstring(i)).c_str());
...
void Renderer::LoadAssets()
{
	...
	res = D3DCompileFromFile(shader_path.c_str(), nullptr, nullptr, "VSMain", "vs_5_0", compile_flag, 0, &vertex_shader, &error);
	if (FAILED(res)) {
		OutputDebugStringA((char*)error->GetBufferPointer());
		ThrowIfFailed(res);
	}

	res = D3DCompileFromFile(shader_path.c_str(), nullptr, nullptr, "PSMain", "ps_5_0", compile_flag, 0, &pixel_shader, &error);
	if (FAILED(res)) {
		OutputDebugStringA((char*)error->GetBufferPointer());
		ThrowIfFailed(res);
	}
	...
	device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, command_allocators[0].Get(), pipeline_state.Get(),
